...
function Jobs.new(options)
  ...
  local function valid(state)
    return type(state) == "table" and state.job_type == "tunnel_miner" and type(state.id) == "string" and type(state.position) == "table" and type(state.slice) == "number" and type(state.distance) == "number"
  end
  function jobs:load()
