...
dashboard:reset(); dashboard:render({ status = "MINING", slice = 50, distance = 100, pattern = "9x9", fuel = 900, inventory_fuel = 37, inventory_fuel_label = "minecraft:coal", loot = 4, capacity = 13, torches = 20, filler = 64, ores = 8, veins = 2, unloads = 1, ore = "Diamond Ore" })
assert(state.rows[1]:find("RalfieOS Miner") and state.rows[3]:find("50%%") and state.rows[5]:find("Pattern: 9x9") and state.rows[6]:find("Inventory: 37 minecraft:coal") and state.rows[10]:find("Diamond Ore"))
...
assert(exhaustedState.rows[2]:find("OUT_OF_FUEL") and exhaustedState.rows[6]:find("Inventory: 37"))
